...
<html>
    ...
    <body id="particles-js">
        ...
        <div class="split left" >
            ...
            $sql = "SELECT SCREEN_NO FROM BOOKING_DETAILS where CUST_ID=1";
        ...
        <div class="split right" >
            ...
            $sql = "SELECT CUST_ID FROM BOOKING_DETAILS where CUST_ID=1";
            $result = mysqli_query($conn, $sql);
            $res= mysqli_fetch_assoc($result);
        
            echo "<ul>BOOKING ID: &nbsp". $res["CUST_ID"]. "</ul>";
            
            $sql = "SELECT MOVIE_NAME FROM BOOKING_DETAILS where CUST_ID=1";
            $result = mysqli_query($conn, $sql);
            $res= mysqli_fetch_assoc($result);
        
            echo "<ul>MOVIE NAME: ". $res["MOVIE_NAME"]. "</ul>";
            
            $sql = "SELECT SCREEN_NO FROM BOOKING_DETAILS where CUST_ID=2";
            $result = mysqli_query($conn, $sql);
            $res= mysqli_fetch_assoc($result);
        
            echo "<ul>SCREEN NO:  &nbsp ". $res["SCREEN_NO"]. "</ul>";
            
            $sql = "SELECT SEAT_RANGE FROM BOOKING_DETAILS where CUST_ID=1";
            $result = mysqli_query($conn, $sql);
            $res= mysqli_fetch_assoc($result);
        
            echo "<ul>SEATS BOOKED:". $res["SEAT_RANGE"]. "</ul>";
            $sql = "SELECT TIME_ID FROM BOOKING_DETAILS where CUST_ID=2";
            $result = mysqli_query($conn, $sql);
            $res= mysqli_fetch_assoc($result);
            if($res["TIME_ID"]==1)
